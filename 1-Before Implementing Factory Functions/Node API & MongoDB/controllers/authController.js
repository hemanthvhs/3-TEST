...
const createSendToken = (user, statusCode, res) => {
    ...
    if (process.env.NODE_ENV === 'production') cookieOptions.secure = true; // For development secure will be fals by default.
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    // If user deletes himself by using the route /deleteMe, then active will be set to false. Therefore for all the find queries querymiddleware will run and filters the users whose active is set to false. Which leads to no user found.
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    // This check is important - Scenario user has changed the password but using the old JWT token
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    await user.save({ validateBeforeSave: false }); // If we dont use validateBeforeSave: false, then validation will trigger. As user object has name,email,password,active (Therefore valdiation passes) but validation fails for confirmPassword as it is required. Since we dont store confirm password in DB therfore we run validateBeforeSave set to false
