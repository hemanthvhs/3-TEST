...
const userSchema = new mongoose.Schema({
    ...
    password: {
        ...
        select: false, // This will make that password field will never appear when we query using /^find()/. But when we create a new user then resposne of that new user from DB includes the password
    },
    confirmPassword: {
        type: String,
...
userSchema.pre('save', async function (next) {
    ...
    // IF PASSWORD IS NEWLY INSERTED OR MODIFIED THEN this.isModified('password') is TRUE
    ...
    this.confirmPassword = undefined; // If a field is set to undefined then it is not saved to DB
...
userSchema.methods.changedPasswordAfter = function (JWTExpiresInTime) {
    if (this.passwordChangedAt) {
        const passwordChangedAtTime = parseInt(
            this.passwordChangedAt.getTime() / 1000, // Converting to seconds since JWTExpiresInTime is in Seconds
            10
        );
